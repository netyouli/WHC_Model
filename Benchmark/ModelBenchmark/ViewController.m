...
- (void)viewDidLoad {
    ...
    SZCategory * sz = [SZCategory whc_ModelWithJson:json keyPath:@"data.2"];
...
- (void)benchmarkGithubUser {
    ...
    @autoreleasepool {
        for (int i = 0; i < count; i++) {
            ...
            // WHC_Model
            [WHCUser whc_ModelWithJson:json];
    ...
    /*------------------- WHC_Model -------------------*/
    {
        ...
        @autoreleasepool {
            for (int i = 0; i < count; i++) {
                WHCUser *user = [WHCUser whc_ModelWithJson:json];
                [holder addObject:user];
            }
        }
        end = CACurrentMediaTime();
        printf("WHC_Model:        %8.2f   ", (end - begin) * 1000);
        ...
        @autoreleasepool {
            for (int i = 0; i < count; i++) {
                NSDictionary *json = [user whc_Dictionary];
                [holder addObject:json];
            }
        }
        end = CACurrentMediaTime();
        if ([NSJSONSerialization isValidJSONObject:[user whc_Dictionary]]) {
            printf("%8.2f   ", (end - begin) * 1000);
        } else {
            printf("   error   ");
        }
...
- (void)benchmarkWeiboStatus {
    ...
    @autoreleasepool {
        for (int i = 0; i < count * 2; i++) {
            // WHCModel
            [WHCWeiboStatus whc_ModelWithJson:json];
    ...
    /*------------------- WHC_Model -------------------*/
    {
        ...
        @autoreleasepool {
            for (int i = 0; i < count; i++) {
                WHCWeiboStatus *feed = [WHCWeiboStatus whc_ModelWithJson:json];
        ...
        @autoreleasepool {
            for (int i = 0; i < count; i++) {
                NSDictionary *json = [feed whc_Dictionary];
                [holder addObject:json];
            }
        }
        end = CACurrentMediaTime();
        if ([NSJSONSerialization isValidJSONObject:[feed whc_Dictionary]]) {
            printf("%8.2f   ", (end - begin) * 1000);
        } else {
            printf("   error   ");
        }
